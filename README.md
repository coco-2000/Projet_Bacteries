...
### Autres
- En général, lorsqu'une méthode virtuelle pure est implémentée par une sous-classe, le mot-clé `override` serait le
bievenue, de sorte que le compilateur puisse vérifier si nécessaire si l'overriding est valide ou pas.
...
N'utilisez les références non-constantes que lorsque vous le devez.

- Dans `PetriDish::update()`, vous devriez utiliser le vecteur `annex` *avant* d'updater vos bactéries afin d'éviter un
décalage dans les updates.

- Faites attention aux cas de division par zéro (e.g. `SimpleBacterium::tentative_basculement()`).
